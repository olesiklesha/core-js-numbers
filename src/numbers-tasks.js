...
function getAverage(value1, value2) {
  return value1 === value2 ? value2 : (value1 + value2) / 2;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const moduleX = Math.hypot(x1, y1);
  const moduleY = Math.hypot(x1, y2);

  return Math.acos(mul === 0 ? 0 : mul / (moduleX * moduleY));
}
...
function toNumber(value, def) {
  return Number.isInteger(Number(value)) ? +value : def;
}
...
function getFibonacciNumber(index) {
  ...
  return Math.round(
    (goldenRatio ** index - (-1 * goldenRatio) ** (-1 * index)) /
      (2 * goldenRatio - 1)
  );
}
